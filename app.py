...
.rebate-display{font-family:monospace;font-size:9px;color:rgba(61,255,160,0.8);padding:4px 8px;background:rgba(61,255,160,0.05);border:1px solid rgba(61,255,160,0.15);margin-bottom:10px;margin-top:3px}
...
<div class="mono" style="background:#12121c;border:1px solid rgba(77,159,255,0.15);padding:5px 10px;font-size:9px;color:#4d9fff;letter-spacing:0.12em;text-transform:uppercase">v4 &#183; 2026</div>
</div>
<div class="card">
...
<div style="margin-bottom:4px"><label class="lbl">Fundos &#8212; Taxa de Adm. m&#233;dia (% a.a.)</label><div class="inp-wrap"><span class="inp-pfx">%</span><input type="number" id="inp-taF" class="inp" value="1.0" step="0.1" min="0" max="5"></div></div>
<div style="margin-bottom:4px"><label class="lbl">Fundos &#8212; % Rebate ao assessor</label><div class="inp-wrap"><span class="inp-pfx">%</span><input type="number" id="inp-rpF" class="inp" value="50" step="5" min="0" max="100"></div></div>
<div class="rebate-display">Rebate efetivo sobre AUM: <span id="rbF-display">0,50%</span> a.a.</div>
<div style="margin-bottom:4px"><label class="lbl">Previd&#234;ncia &#8212; Taxa de Adm. m&#233;dia (% a.a.)</label><div class="inp-wrap"><span class="inp-pfx">%</span><input type="number" id="inp-taP" class="inp" value="0.6" step="0.1" min="0" max="5"></div></div>
<div style="margin-bottom:4px"><label class="lbl">Previd&#234;ncia &#8212; % Rebate ao assessor</label><div class="inp-wrap"><span class="inp-pfx">%</span><input type="number" id="inp-rpP" class="inp" value="50" step="5" min="0" max="100"></div></div>
<div class="rebate-display">Rebate efetivo sobre AUM: <span id="rbP-display">0,30%</span> a.a.</div>
...
const fee=val("inp-fee"),sp=val("inp-sp"),roa=val("inp-roa");
const taF=val("inp-taF"),rpF=val("inp-rpF"),taP=val("inp-taP"),rpP=val("inp-rpP");
const base=rf+fd+pv+co,fD=fee/100,spD=sp/100,roaD=roa/100;
const rbFD=taF/100*rpF/100,rbPD=taP/100*rpP/100;
...
const taF=val("inp-taF"),rpF=val("inp-rpF"),taP=val("inp-taP"),rpP=val("inp-rpP");
const efF=(taF*rpF/100).toFixed(2).replace(".",",");
const efP=(taP*rpP/100).toFixed(2).replace(".",",");
document.getElementById("rbF-display").textContent=efF+"%";
document.getElementById("rbP-display").textContent=efP+"%";
}
function drawPie(){
...
function update(){updateRebateDisplays();drawPie();drawPieTrans();drawResult();drawBars();}
...
tools:[{name:"parse_portfolio",description:"Extrai dados financeiros do extrato",input_schema:{type:"object",properties:{nome:{type:["string","null"],description:"Nome do cliente"},rf:{type:"number",description:"Renda Fixa (CDB, LCI, LCA, LCD, CRI, CRA, Debentures, Tesouro Direto, LFT, LTN, NTN-B, compromissadas)"},fundos:{type:"number",description:"Fundos de investimento abertos identificados por NOMES DESCRITIVOS LONGOS (ex: XP Macro FIC FIM, BTG Pactual RF FIRF, Kapitalo Kappa FIC FIM, Ibiuna Hedge FIM) — inclui FIC, FIM, FIRF, FI RF, Fundo Mutuo. EXCLUIR: (a) fundos com Prev, VGBL, PGBL no nome = sao previdencia; (b) tickers curtos terminados em 11 (HGLG11, KNRI11) = sao FIIs, vao para excluidos"},prev:{type:"number",description:"Previdencia privada: VGBL, PGBL, e fundos com wrapper previdenciario (nomes com Prev, XP Seg Prev, Seg FIC, etc) mesmo que sejam FIC/FIM/FIRF — o indicador e a presenca de Prev mais seguradora no nome"},coe:{type:"number",description:"COE"},excluidos:{type:"number",description:"Ativos identificados por TICKERS CURTOS: acoes (PETR4, VALE3, ITUB4), FIIs com ticker terminado em 11 (HGLG11, KNRI11, XPLG11, MXRF11), ETFs (BOVA11, IVVB11), BDRs (AAPL34). A secao Fundos Listados do extrato XP contem FIIs com tickers terminados em 11. NAO incluir fundos com nomes longos (FIC, FIM, FIRF) — esses sao fundos abertos"},rf_ano1:{type:"number",description:"RF vencendo em ate 12 meses"},rf_ano2:{type:"number",description:"RF vencendo entre 12 e 24 meses"}},required:["nome","rf","fundos","prev","coe","excluidos","rf_ano1","rf_ano2"]}}],
tool_choice:{type:"tool",name:"parse_portfolio"},
messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:b64}},{type:"text",text:"Leia o extrato e chame parse_portfolio. Regras de classificacao: (1) rf = CDB, LCI, LCA, LCD, CRI, CRA, Debentures, Tesouro Direto, LFT, LTN, NTN-B, compromissadas. (2) fundos = fundos identificados por NOMES DESCRITIVOS LONGOS sem Prev/VGBL/PGBL: FIC, FIM, FIRF, FI RF, Fundo Mutuo — ex: XP Macro FIC FIM, BTG Pactual RF FIRF, Kapitalo Kappa. (3) prev = tudo com Prev, VGBL, PGBL ou nome de seguradora no fundo: Riza Lotus Prev, Sulamerica Prev, XP Seg VGBL etc. (4) excluidos = SOMENTE ativos com TICKER CURTO: acoes (PETR4, VALE3), FIIs (ticker terminado em 11: HGLG11, KNRI11, XPLG11), ETFs, BDRs. A secao Fundos Listados do XP tem FIIs com ticker terminado em 11 — esses vao para excluidos. Fundos com nomes longos (FIC/FIM/FIRF) NUNCA vao para excluidos. (5) Ponto como decimal. (6) Nao encontrado = 0. CRITICO: cada ativo em UMA categoria apenas."}]}]
})});
