...
async function processPDF(file){
  ...
  try{
    const resp=await fetch("/proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      system:"Você é um extrator de dados de investimento. Analise o documento e extraia os totais por classe de ativo. Retorne SOMENTE um objeto JSON válido, sem texto antes ou depois.\n\nFormato obrigatório (valores numéricos sem formatação, use 0 se a classe não aparecer):\n{\"nome\":\"nome do cliente ou null\",\"rf\":number,\"fundos\":number,\"prev\":number,\"coe\":number,\"excluidos\":number}\n\nRegras de classificação:\n- rf: Renda Fixa em geral — CDB, LCI, LCA, LCD, CRI, CRA, Debêntures, NTN-B, Tesouro Direto, LFT, LTN, poupança, compromissadas\n- fundos: Fundos de Investimento de qualquer tipo (exceto previdência)\n- prev: Previdência Privada — VGBL, PGBL\n- coe: COE\n- excluidos: Renda Variável — Ações, FIIs, ETFs, BDRs, opções\n\nSe o documento não contiver dados financeiros de investimento (ex: nota fiscal, artigo, receita): {\"erro\":\"documento nao reconhecido\"}\n\nNUNCA inclua texto fora do JSON. NUNCA use markdown.",
